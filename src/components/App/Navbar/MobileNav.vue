<template>
  <nav class="nav flex flex-wrap items-center justify-between relative">
    <input class="menu-btn hidden" type="checkbox" id="menu-btn" />
    <label class="menu-icon" for="menu-btn">
      <span class="navicon bg-grey-darkest flex items-center relative"></span>
    </label>
    <router-link :to="{ path: '/' }" class="home-logo" @click="closeMenu">
      <svg
        ...
        viewBox="0 0 76 19"
        fill="none"
        class="site-nav-logo fill-current"
      >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
    ...
    <ul class="menu border-b flex justify-end list-reset m-0 w-full mt-16">
      <li v-for="(menu, index) in menus" :key="index" class="">
        <div
          @click="toggleExpansion(menu, index)"
          class="flex justify-between px-4 py-3 no-underline text-grey-darkest font-bold text-black"
        >
          <span class="">{{ menu.title }}</span>
          <span
            v-if="menu.subs"
            :class="{ 'transform rotate-180': isExpanded(index) }"
            class="transtion duration-150"
          >
            <svg
              width="16"
              height="16"
        ...
        <transition name="fadeDown">
          <ul
            ...
            class="border-l border-grey-300 ml-4 pl-6 transition duration-100 ease-in-out"
          >
            <li
              v-for="(subMenu, i) in menu.subs"
              :key="i"
              class="py-3 text-black"
            >
              {{ subMenu.title }}
            </li>
          </ul>
...
export default {
  ...
  setup() {
    ...
    function isExpanded(key) {
      return expandedGroup.value.indexOf(key) !== -1;
    }
    function toggleExpansion(menu, key) {
      if (menu.subs) {
        if (isExpanded(key))
          expandedGroup.value.splice(expandedGroup.value.indexOf(key), 1);
        else expandedGroup.value.push(key);
        return;
      }
      closeMenu();
    ...
    return { isExpanded, toggleExpansion, expandedGroup, closeMenu };
  },
};
</script>
...
.home-logo {
  left: calc(50% - 40px);
  @apply absolute top-0 h-16 w-20 flex justify-center;
}
@media (max-width: 767px) {
  ...
  .menu-icon {
    ...
    left: 24px;
    top: 30px;
    @apply block cursor-pointer select-none absolute;
  }
